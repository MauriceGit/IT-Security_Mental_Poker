...
public class SRATest extends SimpleTest 
{
    ...
    private void testStrictPKCS1Length(SRAKeyParameters pubParameters, SRAKeyParameters privParameters)
    {
        ...
        {
            fail("SRA: failed - exception " + e.toString(), e);
        }
        ...
        eng = new PKCS1Encoding(new SRAEngine());
    ...
    private void testTruncatedPKCS1Block(SRAKeyParameters pubParameters, SRAKeyParameters privParameters)
    {
        checkForPKCS1Exception(pubParameters, privParameters, truncatedDataBlock, "block truncated");
    }
    
    private void testDudPKCS1Block(SRAKeyParameters pubParameters, SRAKeyParameters privParameters)
    {
        checkForPKCS1Exception(pubParameters, privParameters, dudBlock, "unknown block type");
    }

    private void testWrongPaddingPKCS1Block(SRAKeyParameters pubParameters, SRAKeyParameters privParameters)
    {
        checkForPKCS1Exception(pubParameters, privParameters, incorrectPadding, "block padding incorrect");
    }
    
    private void testMissingDataPKCS1Block(SRAKeyParameters pubParameters, SRAKeyParameters privParameters)
    {
        checkForPKCS1Exception(pubParameters, privParameters, missingDataBlock, "no data in block");
    }

    private void checkForPKCS1Exception(SRAKeyParameters pubParameters, SRAKeyParameters privParameters, byte[] inputData, String expectedMessage)
    {
        AsymmetricBlockCipher   eng = new SRAEngine();
    ...
    private void testOAEP(SRAKeyParameters pubParameters, SRAKeyParameters privParameters)
    {
        //
        // OAEP - public encrypt, private decrypt
        //
        AsymmetricBlockCipher eng = new OAEPEncoding(new SRAEngine());
    ...
    {
        AsymmetricBlockCipher eng = new PKCS1Encoding(new SRAEngine());
    ...
    public void performTest() throws Exception {
        ...
        SRAKeyParameters    pubParameters = new SRAKeyParameters(false, p, q, pubExp);
        SRAKeyParameters    privParameters = new SRAKeyParameters(true, p, q, pubExp);
        //RSAKeyParameters    privParameters = new SRAPrivateCrtKeyParameters(mod, pubExp, privExp, p, q, pExp, qExp, crtCoef);
        ...
        {
            fail("encryption failed - exception " + e.toString(), e);
        }
        ...
        {
            fail("decryption failed - exception " + e.toString(), e);
        }
        ...
        SRAKeyGenerationParameters  genParam = new SRAKeyGenerationParameters(BigInteger.valueOf(0x11), new SecureRandom(), 768, 25);
        ...
        if (((SRAKeyParameters)pair.getPublic()).getModulus().bitLength() < 768)
        {
        ...
        genParam = new SRAKeyGenerationParameters(BigInteger.valueOf(0x11), new SecureRandom(), 1024, 25);
        ...
        if (((SRAKeyParameters)pair.getPublic()).getModulus().bitLength() < 1024)
        {
        ...
        {
            pair = pGen.generateKeyPair();
            SRAKeyParameters privKey = (SRAKeyParameters) pair.getPrivate();
            BigInteger pqDiff = privKey.getP().subtract(privKey.getQ()).abs();

            if (pqDiff.bitLength() < 5)
            {
                fail("P and Q too close in SRA key pair");
            }
        }
        ...
        try
        {
            new SRAEngine().processBlock(new byte[]{ 1 }, 0, 1);
